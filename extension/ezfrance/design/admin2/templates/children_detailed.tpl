...
{if and( $node.is_container,  $node.can_create)}
    ...
    {foreach $can_create_classes as $group}
        ...
        {foreach $group.items as $can_create_class}
            ...
            {ldelim} text: "<span class='eziconize'>{$can_create_class.identifier|class_icon('small')|wash('javascript')}</span>&nbsp;{$can_create_class.name|wash()}", value: {$can_create_class.id} {rdelim}
            {/if}

...
if ( $http->hasPostVariable( "eZOptionUpdate" ) )
{


	$eZOptionUpdate = $http->postVariable( "eZOptionUpdate" );

	foreach($eZOptionUpdate as $eZProductCollectionItemID  => $tmp)
	{
		//echo "AddToBasket_OptionList_" . $eZProductCollectionItemID;
		$http->setSessionVariable("AddToBasket_OptionList_" . $eZProductCollectionItemID, $tmp);
	}
}
// Fin du hack
*/
...
//print_r($items);
print_r($items[0][item_object]->attribute('option_list'));
...
/*
	...
	print_r($eZOptionUpdate);
	foreach($eZOptionUpdate as $eZProductCollectionItemID  => $tmp)
	{

		$optionID = array_shift(array_values($array));
...
*/
                        ...
                        /*eZProductCollectionItemOption:create();
...
if ( $http->hasPostVariable( "StoreChangesButton" ) )
{
	...
	if ( $http->hasPostVariable( "eZOptionUpdate" ) )
	{
		...
		foreach($basket->attribute('items') as $item)
		{
			$itemID = $item['id'];
			if( array_key_exists($itemID , $eZOptionUpdate))
			{
					// Premier element du tableau

					$oldCollectionItem = eZProductCollectionItem::fetch($itemID);

					$ObjectID = $oldCollectionItem->attribute('contentobject_id');
					$listQuantity = $http->postVariable( "ProductItemCountList" );
					$quantity = $listQuantity[$itemID];

					$OptionList = $eZOptionUpdate[$itemID];
    ...
    $newItemCountList = array();
    $newItemIDList = array();
    foreach($itemIDList as $key => $val){
        $newItemIDList[] = $val;
        $newItemCountList[] = $itemCountList[$key];
    }

    $itemIDList = $newItemIDList;
    $itemCountList = $newItemCountList;
...
if ( $http->hasPostVariable( "ContinueShoppingButton" ) )
{
    ...
    if ( is_array( $itemCountList ) && is_array( $itemIDList ) && count( $itemCountList ) == count( $itemIDList ) && is_object( $basket ) )
    {
       ...
       // $i = 0;
        ...
        foreach ( $itemIDList as $id )
        {
            if ( !is_numeric( $itemCountList[$id] ) or $itemCountList[$id] <= 0 )
            {
                $itemCountError = true;
            }
            else
            {
                ...
                if ( is_object( $item ) && $item->attribute( 'productcollection_id' ) == $productCollectionID )
                {
                    $item->setAttribute( "item_count", $itemCountList[$id] );
                    $item->store();
                }
            }
           // $i++;
